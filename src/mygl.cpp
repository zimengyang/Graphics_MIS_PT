...
//    //draw pixels
//    unsigned char* pixBuffer = new unsigned char[scene.camera.width*scene.camera.height*3];

//    for(int i=0;i<scene.camera.width;i++)
//        for(int j=0;j<scene.camera.height;j++)
//        {
//            int index = i * scene.camera.height + j;
//            pixBuffer[index * 3 + 0] = 255;
//            pixBuffer[index * 3 + 1] = 0;
//            pixBuffer[index * 3 + 2] = 0;
//        }

//    glDrawPixels(scene.camera.width,scene.camera.height,GL_RGB,GL_UNSIGNED_BYTE,pixBuffer);
...
void MyGL::RaytraceScene()
{
    ...
    unsigned char* buffer = new unsigned char[scene.camera.width*scene.camera.height*3];
    glReadPixels(100,100,scene.camera.width,scene.camera.height,GL_RGB,GL_UNSIGNED_BYTE,buffer);
    for(int i=0;i<scene.camera.width;i++)
        for(int j=0;j<scene.camera.height;j++)
        {
            int index = i*scene.camera.width+j;
            float r = float(buffer[index*3+0]) / 255.0f;
            float g = float(buffer[index*3+1]) / 255.0f;
            float b = float(buffer[index*3+2]) / 255.0f;

            scene.film.pixels[i][j] = glm::vec3(r,g,b);
        }
...
#define MULTITHREADED
    ...
    for(unsigned int Y = 0; Y < y_block_count; Y++)
    {
        ...
        for(unsigned int X = 0; X < x_block_count; X++)
        {
            ...
            render_threads[Y * x_block_count + X] = new RenderThread(x_start, x_end, y_start, y_end, scene.sqrt_samples, integrator->getMaxDepth(), &(scene.film), &(scene.camera), integrator);
